...
RUN apt-get update && apt-get install -y curl ca-certificates python3 git make g++ procps gosu --no-install-recommends && rm -rf /var/lib/apt/lists/*
...
# Run as node user (UID 1000 = host jakem) so ~/.claude mounts are readable
USER node
ENV PORT=3000
EXPOSE 3000
