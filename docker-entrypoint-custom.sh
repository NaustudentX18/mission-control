...
# Skip chmod if running as non-root (volume mounts may have different ownership)
if [ -f "$SECRETS_FILE" ] && [ "$(id -u)" = "0" ]; then
...
# Running as node (UID 1000) from Dockerfile USER directive — no switch needed
printf '[entrypoint] Running as node user (uid=%d, home=%s)\n' "$(id -u)" "$(eval echo ~)"
exec node server.js
